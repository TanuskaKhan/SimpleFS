...
int filename_exists(FILE *image, const char *filename)
{
    ...
    /* TODO: STUDENT CODE START */
    if (fseek(image, (long)ROOT_DATA_BLOCK * BLOCK_SIZE, SEEK_SET) != 0) {
        return 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    /* TODO: STUDENT CODE START */
    {
        ...
        for (int i = 0; i < required_blocks; i++) {
            size_t bytes_to_read = (remaining > BLOCK_SIZE) ? BLOCK_SIZE : (size_t)remaining;
            memset(buffer, 0, sizeof(buffer));

            if (fread(buffer, 1, bytes_to_read, source) != bytes_to_read) {
